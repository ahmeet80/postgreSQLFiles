...
-- 4) id'si 5 olan personelin isim, id, yaş ve email bilgilerini listeleyiniz
...
select * from personel where id not IN (39,48,54);
...
select * from personel order by yas; -- dogal sıralaması kucukten buyuge (def asc)
...
select * from personel order by yas desc; -- desc buyukten kucuge siralama
...
select * from personel order by maas desc limit 3; -- limit 3 sıralamada ilk 3 ü alıyor
