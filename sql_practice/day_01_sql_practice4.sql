...
where isyeri
in(select marka_isim from markalar where calisan_sayisi>15000);	-- where isyeri--> baglanti noktasi
-- in kullanacaksaniz parantez icine aldiginiz sorgu birden fazla deger dondurmelidir

select * from calisanlar;-- calisanlar tablosu icin birlesme isteri stunu
select * from markalar; -- markalar tablosu icin birlesme yeri marka_isim stunu

-- SORU2: marka_id’si 101’den büyük olan marka çalişanlarinin isim, maaş ve 
-- şehirlerini listeleyiniz. 


-- SORU3: Ankara’da calisani olan markalarin marka id'lerini ve 
