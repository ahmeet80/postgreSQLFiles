...
INSERT INTO adresler VALUES('10012','Ağa Sok', '30.Cad.','Antep'); -- parentta olmdığı için ekleme yapmaz 
...
drop table calisanlar; -- primary key ve onu referans alan foreign key olduğu sürece tablo silinemez
-- ON DELETE CASCADE --
--Her defasında önce child tablodaki verileri silmek yerine 
--ON DELETE CASCADE silme özelliğini aktif hale getirebiliriz
--Bunun için FK(Foreign Key) olan satırın en sonuna ON DELETE CASCADE komutunu yazmamız yeterli
...
INSERT INTO talebeler VALUES(128, 'Mustafa Can', 'Hasan',45);-- yukarda 60'i sinir olarak belirledigimiz icin bunu eklemez
...
--ismi Mustafa Bak olan talebelerin tum bilgierini getirin
...
Hem sınav1 hemde sınav2 alanı, 50'den büyük olan kayıtları listeler
    ...
    select * From matematik sınav1>50 OR sınav2>50 
...
-- Derya Soylu ile Yavuz Bal arasindaki 
...
--	   tek komutta yazabilme imkanı verir
--Farklı sütunlar için IN kullanılamaz

--id'si 1001,1002ve 1004 olan personelin bilgilerini listele
select * from personel WHERE id = '1001' or id = '1002' or id = '1004';

--2.Yol
select * from personel WHERE id IN ('1001','1002','1003','1004');
