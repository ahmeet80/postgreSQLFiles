...
--SIMILAR TO ile
select * from kelimeler where kelime similar to 'h[a-z,A-Z,0-9]t'; -- kontrol et
--LIKE
...
--SIMILAR TO ile
select kelime  from kelimeler where kelime similar to 'h[a-e](.*)%t'; -- .*son karakter demek
--REGEX ile
select kelime  from kelimeler where kelime ~* 'h[a-e](.*)t';

--İlk karakteri 's', 'a' veya 'y' olan "kelime" değerlerini çağırın.
select * from kelimeler where kelime ~ '^[say](.*)'; -- ^ başlangıcı temsl eder
